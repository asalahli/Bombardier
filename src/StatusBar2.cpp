...
StatusBar2::StatusBar2(Screen& screen)
    : GameObject(screen)
{
    mVertices.resize(24);
    ...
    // Bar
    getScreen().getGame().getResourceManager().loadSprite(
        this, 3, "barHorizontal_red_left.png", 0, 0);

    getScreen().getGame().getResourceManager().loadSprite(
        this, 4, "barHorizontal_red_mid.png", mVertices[13].position.x, mVertices[13].position.y);

    mVertices[17].position.x = offset+100;
    mVertices[18].position.x = offset+100;

    getScreen().getGame().getResourceManager().loadSprite(
        this, 5, "barHorizontal_red_right.png", mVertices[17].position.x, mVertices[17].position.y);

    setScale(0.5, 0.5);
    setPosition(-50, -70);
}

void StatusBar2::setHealth(int health) {
